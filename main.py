...
def extract_text_from_pdf_full_read(pdf_input):
    """
    Extract text from all pages of a PDF file or BytesIO stream.

    Args:
        pdf_input (str | BytesIO): File path or BytesIO stream of the PDF.

    Returns:
        dict: {pdf_name: {page_number: page_text, ...}}
    """
    pdf_text = {}
    ...
    try:
        ...
        if isinstance(pdf_input, str):
            ...
            # File-like object (e.g., BytesIO from S3)
            pdf_name = "from_stream.pdf"
            pdf_input.seek(0)  # Ensure stream is at start
            doc = fitz.open(stream=pdf_input.read(), filetype="pdf")
        ...
        for page_number in range(total_pages):
            try:
                page = doc.load_page(page_number)
                page_text = clean_extracted_text(page.get_text())
                pdf_text[pdf_name][page_number + 1] = page_text  # 1-based page numbers
            except Exception as e:
                print(f"[ERROR] Failed to read page {page_number + 1}: {e}")
                continue
...
@app.post("/api/train_model_full_pdf_read")
        ...
        for obj in response["Contents"]:
            ...
            if key.lower().endswith(".pdf") and not key.endswith("/"):
                try:
                    ...
                    print(f"[INFO] Extracting text from entire PDF '{key}'")
                    pdf_data = extract_text_from_pdf_full_read(file_stream)  # process full PDF
...
async def whatsapp_webhook(request: Request):
    ...
    if not validator.validate(full_url, raw_body, twilio_signature):
        ...
        print("  2. Parameter encoding issues (+ vs %20)")
